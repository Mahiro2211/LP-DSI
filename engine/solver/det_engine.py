...
def _compute_encoder_transformer_grad_percentage(model: torch.nn.Module) -> float:
    ...
    for name, param in model.named_parameters():
        ...
        total_l1 += val
        # Strip DDP ('module.') / torch.compile ('_orig_mod.') prefixes so the
        # AIFI (encoder.encoder) match works with and without distributed
        # wrapping; without this the share is always 0 in single-process runs
        # and GAM needlessly resets the distill weight to its default.
        pname = name
        while pname.startswith(('module.', '_orig_mod.')):
            pname = pname.split('.', 1)[1]
        if pname.startswith('encoder.encoder'):
            enc_l1 += val
