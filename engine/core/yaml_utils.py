...
def load_config(file_path, cfg=None):
    """load config
    """
    # a fresh accumulator per top-level call; a mutable default argument here
    # would leak keys from previously loaded configs into later ones (e.g.
    # stale teacher_model keys when several configs share one process).
    if cfg is None:
        cfg = {}
