...
from ..rtv4.croma_teacher import CROMATeacherModel
...
class YAMLConfig(BaseConfig):
    ...
    def teacher_model(self, ) -> torch.nn.Module:
        if self._teacher_model is None and 'teacher_model' in self.yaml_cfg:
            ...
            # Explicit `teacher_model: null` (e.g. the no-distillation ablation
            # layered on a distillation config) disables the teacher entirely.
            if teacher_model_cfg is None:
                return super().teacher_model

            model_type = teacher_model_cfg.pop('type', None)
            if model_type == "CROMATeacherModel":
                try:
                    self._teacher_model = CROMATeacherModel(**teacher_model_cfg)
                    teacher_model_cfg['type'] = model_type
                    if dist_utils.is_main_process():
                        print("Successfully loaded and configured CROMA Teacher Model.")
                except Exception as e:
                    print(f"Error creating CROMATeacherModel: {e}")
                    teacher_model_cfg['type'] = model_type
                    raise
            else:
                print(
                    f"Configured teacher_model type '{model_type}' is not "
                    f"'CROMATeacherModel'.")
