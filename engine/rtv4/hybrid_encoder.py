...
from .distill_projector import MLPProjector
...
class HybridEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        # feature_projector (training-only; the 'linear' branch must stay an
        # nn.Sequential(Linear) so existing checkpoints keep loading)
        self.feature_projector = None
        if self.distill_teacher_dim > 0:
            if distill_projector_type == 'linear':
                self.feature_projector = nn.Sequential(
                        nn.Linear(hidden_dim, self.distill_teacher_dim),
                        # nn.GELU(),
                    )
            elif distill_projector_type == 'mlp':
                self.feature_projector = MLPProjector(hidden_dim, self.distill_teacher_dim)
            else:
                raise ValueError(
                    f"distill_projector_type must be one of ['linear', 'mlp'], "
                    f"got '{distill_projector_type}'")
    ...
    def _reset_parameters(self):
        if self.eval_spatial_size:
            for idx in self.use_encoder_idx:
                ...
                # Must be a registered buffer so it follows module.to(device);
                # persistent=False keeps it out of state_dict for checkpoint compat.
