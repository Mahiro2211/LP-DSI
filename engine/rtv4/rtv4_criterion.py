...
from .distill_modules import cosine_alignment_loss, low_pass_alignment_loss
...
class RTv4Criterion(nn.Module):
    ...
    def __init__(self, \
        ...
        assert distill_mode in ('cosine', 'lp')
    ...
    def loss_distillation(self, outputs, targets, indices, num_boxes, **kwargs):
        ...
        # Under AMP the student feature map comes from inside autocast (fp16) while the
        # teacher feature map is produced outside autocast (fp32); this criterion runs
        # with autocast disabled, so dtypes must be harmonized here or backward raises
        # "Found dtype Float but expected Half".
        student_feature_map = student_feature_map.float()
        teacher_feature_map = teacher_feature_map.float()
        ...
        if (H_s, W_s) != (H_t, W_t):
            if not self._resize_warned:
                ...
                _logger.warning(
                    f"[RTv4Criterion] Resizing teacher feature map from {H_t}x{W_t} to student's {H_s}x{W_s} for distillation (logged once).")
            teacher_feature_map = F.interpolate(teacher_feature_map,
                                                size=(target_h, target_w),
                                                mode='bilinear',
                                                align_corners=False)

        if self.distill_mode == 'lp':
            # Low-pass alignment: only the smoothed (Haar LL) band is aligned,
            # ignoring speckle-dominated high-frequency responses.
            loss_distill = low_pass_alignment_loss(student_feature_map, teacher_feature_map)
        else:
            loss_distill = cosine_alignment_loss(student_feature_map, teacher_feature_map)
        ...
        for loss_name in self.losses:
            ...
            if loss_name == 'distill':
                ...
                if 'loss_distill' in l_dict and l_dict['loss_distill'] != 0:
                    dynamic_weight = self.weight_dict.get('loss_distill', 0.0)
        ...
        if 'aux_outputs' in outputs:
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                if 'local' in self.losses:  # only work for local loss
                    aux_outputs['up'] = aux_outputs.get('up', outputs['up'])
                    aux_outputs['reg_scale'] = aux_outputs.get('reg_scale', outputs['reg_scale'])
